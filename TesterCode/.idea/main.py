...
#La funzione prende in ingresso algtype, scelto in base al bottone cliccato sul menù. Vedere codice dell'interfaccia grafica
def algorithmcommand(algorithm_name) :
    ...
    #Siccome A è stato sostituito col suo corrispettivo ordinato, A1 è la copia precedente di A non ordinato.
...
def inseriscivett() :
    ...
    try :
        if  not (bool(E2.get().strip())  ) : # se è vuota
            raise ValueError
...
def leggifile():
    ...
    try :
        if fl != '': #se il nome del file è valido
...
#Si è ritenuto necessario utilizzare i thread poiché tkinter (per l'interfaccia grafica) è single Threaded,
#questo può comportare che l'interfaccia grafica si blocchi finchè un'operazione non viene eseguita, con conseguenti
...
#La funzione prende in ingresso i, che è il comando associato al bottone cliccato nell'interfaccia.
#Per ragioni di leggibilità, il nome del comando "i" è lo stesso nome del bottone nell'interfaccia grafica.
...
#La funzione prende in ingresso il tipo di algoritmo da testare e in più :
